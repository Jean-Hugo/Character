...
class Person: 
    """Class that creates the Person Object"""
    id: str = None
    name: str = None
    surname: str = None
    gender: str = None
...
def get_split_person_details(person_details: str, split_char: str = None):
    """Returns a list from the person details supplied"""
    if split_char is None: # If no split char then make it a space
        split_char = " "
    try:
        # Try splitting the person details
        splitted_person_details = person_details.split(split_char)
...
def format_person_details(splitted_details: list, format: int = 0)->list:
    """This fucntion returns the person details in a list form so that the Person object can be created in
    the Person Class
    >>> split_details = format_person_details([gender, name, surname], 0)"""
    # SETTING FORMAT
    person_format = None
...
def create_list(filename: str):
    ...
    ### 1. Loads the file
    ### 2. creates the person details list
    ### 3. formats the person details list
    ### 4. return the formated person details list"""
    ...
    for person in raw_people_data:  # Loop through the person details
        split_details = get_split_person_details(
            person, " ")  # Split the person details
        formated_split_details = format_person_details(
            split_details, 0)  # Put the details in the right order
        # Add the details to the list // list of list
        people_list.append(formated_split_details)
    return people_list
...
def ux_charcter():
    ...
    def setup_actions():  # Sets up things that happen
        push_me_button.pressed.connect(lambda: change_line_edit(
            line_result, "You pushed the button!"))

    display_window = window()
    push_me_button = button(display_window, "Push me!")
    line_result = lineEdit(display_window, "I'm the placeholder text��")
    person_table = table(
        display_window, retrieve_person_objects("names_list.txt"))
    setup_window()
    setup_actions()
    app.exec_()
...
def get_person_object_list(loaded_person_details: list):
    ...
    for person in loaded_person_details:
        ...
        surname = person[1]
        gender = person[2]
        person_objects_list.append(
            Person(name=name, surname=surname, gender=gender))
...
def terminal_ui(loaded_person_details):
    ...
    if (input("Do you want to display?(Y/N)") == "Y"):
        while True:
            ...
            if option == "1":
                # One liner advance
                print_all = [print(name) for name in loaded_person_details]
                break
            elif option == "2":
                print_names_only = [print(name[0])
                                    for name in loaded_person_details]
                break
            elif option == "3":
                print_names_gender = [print(name[0], name[2])
                                      for name in loaded_person_details]
                break
            else:
...
def retrieve_person_objects(filename: str):
    ...
    person_objects = get_person_object_list(
        loaded_person_details=loaded_person_details)
